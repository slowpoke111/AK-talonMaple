...
public class AutoAlignCommand extends Command {
    ...
    public AutoAlignCommand(Drive drive, Vision vision) {
        this.m_Swerve = drive;
        this.m_Vision = vision;
        addRequirements(drive, vision);
    }

    public AutoAlignCommand(Drive drive, Vision vision, int ID) {
        this.m_Swerve = drive;
        this.m_Vision = vision;
    ...
    public void execute(){
        ...
        if (!usePID) {
            new PIDAlignCommand(m_Swerve, new Pose2d(m_Swerve.getPose().getTranslation(),targetPose.getRotation())).withTimeout(0.5).withInterruptBehavior(InterruptionBehavior.kCancelSelf).andThen(firstAlign).schedule();
        }
        else {
